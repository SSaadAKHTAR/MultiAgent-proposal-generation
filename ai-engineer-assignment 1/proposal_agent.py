...
class ProposalAgent:
    ...
    def generate_proposal(self, intake_text: str, matrix: ClientMatrix, feedback_history: str = "", prior_draft: str = None) -> str:
        matrix_json = matrix.model_dump_json(indent=2)
        
        # ORIGINAL PROMPT (WITH TOOL):
        # system_instruction = (
        ...
        system_instruction = (
            ...
            "be placed in the 'Open Questions' section and explicitly addressed as areas needing clarification. "
            "Do NOT present them as confident statements elsewhere in the proposal.\n"
            "3. If feedback_history is provided, you MUST incorporate those changes. Change only what the "
            "feedback specifically calls for, and keep every other section consistent with the prior draft — "
            "do not silently rewrite unrelated sections between iterations.\n"
            "4. Do not introduce facts, figures, or claims that are not present in the intake or Client Matrix. "
            "Where a detail is genuinely missing, say so in Open Questions rather than inventing specifics.\n"
        )
        ...
        if prior_draft:
            prompt += f"PRIOR PROPOSAL DRAFT:\n{prior_draft}\n\n"
            
        if feedback_history:
            prompt += f"FEEDBACK DIRECTIVES:\n{feedback_history}\n\n"

        # ORIGINAL CALL (WITH TOOL):
        ...
        # ACTIVE CALL (WITHOUT TOOL):
        return self.llm.generate_text(prompt=prompt, system_instruction=system_instruction, model="gemini-3.1-pro-preview", agent_name="ProposalAgent")
